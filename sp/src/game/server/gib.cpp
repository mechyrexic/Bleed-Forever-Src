...
LINK_ENTITY_TO_CLASS(giblet, CGiblet);

CGiblet::CGiblet()
{
	m_iBloodColor = BLOOD_COLOR_RED;
...
void CGiblet::SetBleedTimes(int iCount)
{
	m_iBleedTimes = iCount;
}

void CGiblet::SetBloodColor(int color)
{
	m_iBloodColor = color;
}

void CGiblet::SetBleedTime(float flTime)
{
	m_flBleedTime = gpGlobals->curtime + flTime;
}
...
void CGiblet::Think()
{
	if (m_iBleedTimes < 1)
	{
		SetContextThink( &CBaseEntity::SUB_RemoveWhenNotVisible, gpGlobals->curtime, "SUB_RemoveWhenNotVisible" );
		SetNextThink(gpGlobals->curtime);
		return;
	}
	if (m_flBleedTime < gpGlobals->curtime)
	{
		UTIL_BloodImpact(WorldSpaceCenter(), vec3_origin, m_iBloodColor, 32);
		m_flBleedTime = gpGlobals->curtime + 0.5f;
		m_iBleedTimes--;
	}
	SetNextThink(gpGlobals->curtime + 0.1f);
}

void CGiblet::VPhysicsCollision(int index, gamevcollisionevent_t* pEvent)
{
	if (pEvent->collisionSpeed < 50.0f)
	{
		return;
	}
	Vector vecPoint;
	pEvent->pInternalData->GetContactPoint(vecPoint);
	UTIL_BloodImpact(vecPoint, vec3_origin, m_iBloodColor, 32);
	trace_t tr;
	Vector vecSpot;
	Vector vecNormal;
	pEvent->pInternalData->GetContactPoint(vecSpot);
	pEvent->pInternalData->GetSurfaceNormal(vecNormal);
	UTIL_TraceLine(vecSpot, vecSpot + vecNormal * 2, CONTENTS_SOLID, this, COLLISION_GROUP_NONE, &tr);
	if (tr.m_pEnt && tr.fraction != 1.0)
	{
		UTIL_BloodDecalTrace(&tr, m_iBloodColor);
	}
