...
const char* szHGibNames[14] =
{
	"models/gibs/humanoid/humanoidgibhead.mdl",
	"models/gibs/humanoid/humanoidgibtorso.mdl",
	"models/gibs/humanoid/humanoidgibleftfoot.mdl",
	"models/gibs/humanoid/humanoidgibrightfoot.mdl",
	"models/gibs/humanoid/humanoidgibleftlowerleg.mdl",
	"models/gibs/humanoid/humanoidgibrightlowerleg.mdl",
	"models/gibs/humanoid/humanoidgibleftupperleg.mdl",
	"models/gibs/humanoid/humanoidgibrightupperleg.mdl",
	"models/gibs/humanoid/humanoidgibleftupperarm.mdl",
	"models/gibs/humanoid/humanoidgibrightupperarm.mdl",
	"models/gibs/humanoid/humanoidgibleftlowerarm.mdl",
	"models/gibs/humanoid/humanoidgibrightlowerarm.mdl",
	"models/gibs/humanoid/humanoidgiblefthand.mdl",
	"models/gibs/humanoid/humanoidgibrighthand.mdl",
};

const char* szHGibBones[14] =
{
...
bool  CAI_BaseNPC::Event_Gibbed(const CTakeDamageInfo& info)
{
	...
	trace_t hgtr;
	Vector vecDamageDir = info.GetDamagePosition() + info.GetDamageForce();
	VectorNormalize(vecDamageDir);
	for (int i = 0; i < 24; i++)
	{
		trace_t tr;
		UTIL_TraceLine(WorldSpaceCenter(), WorldSpaceCenter() + Vector(random->RandomFloat(-75.0f, 75.0), random->RandomFloat(-75.0f, 75.0f), random->RandomFloat(-75.0f, 75.0f)), CONTENTS_SOLID, this, COLLISION_GROUP_NONE, &tr);
		if (tr.fraction != 1.0)
		{
			UTIL_BloodDecalTrace(&tr, BloodColor());
			//debugoverlay->AddLineOverlay(tr.startpos, tr.endpos, 255, 255, 255, true, 5.0f);
		}
	}
	for (int i = 0; i < random->RandomInt(3,8); i++)
	{
		trace_t tr;
		UTIL_TraceLine(WorldSpaceCenter(), WorldSpaceCenter() + Vector(random->RandomFloat(-75.0f, 75.0), random->RandomFloat(-75.0f, 75.0f), -100.0f), CONTENTS_SOLID, this, COLLISION_GROUP_NONE, &tr);
		if (tr.fraction != 1.0)
		{
			UTIL_BloodDecalTrace(&tr, BloodColor());
			//debugoverlay->AddLineOverlay(tr.startpos, tr.endpos, 255, 0, 0, true, 5.0f);
		}
	}
	for (int i = 0; i < 16; i++)
	{
		...
		Vector vecNormalizedMins, vecNormalizedMaxs, vecPoint;

		CollisionProp()->WorldToNormalizedSpace(vecAbsMins, &vecNormalizedMins);
		CollisionProp()->WorldToNormalizedSpace(vecAbsMaxs, &vecNormalizedMaxs);
		CollisionProp()->RandomPointInBounds(vecNormalizedMins, vecNormalizedMaxs, &vecPoint);
		//SpawnBlood(info.GetDamagePosition() + Vector(random->RandomFloat(-5.0f, 5.0f), random->RandomFloat(-5.0f, 5.0f), random->RandomFloat(-5.0f, 5.0f)), info.GetDamageForce(), BloodColor(), info.GetDamage());
		SpawnBlood(vecPoint, info.GetDamageForce(), BloodColor(), info.GetDamage());
	}
	for (int i = 0; i < 3; i++)
	{
		UTIL_BloodSpray(WorldSpaceCenter(), vecDamageDir, BloodColor(), 24, FX_BLOODSPRAY_ALL);
		UTIL_BloodDrips(WorldSpaceCenter(), vecDamageDir, BloodColor(), 24);
	}
	Vector vecOrigin;
	QAngle vecAngle;
	...
	/*CreateRagGib(szHGibNames[0], WorldSpaceCenter() + Vector(0, 0, 30), GetAbsAngles(), vecDamageDir, 20.0f, IsOnFire()); //head
	CreateRagGib(szHGibNames[1], WorldSpaceCenter() + Vector(0, 0, 15), GetAbsAngles(), vecDamageDir, 20.0f, IsOnFire()); //torso
	EntityToWorldSpace(Vector(0, 4, 3), &vecOut);
	CreateRagGib(szHGibNames[2], vecOut, GetAbsAngles(), vecDamageDir, 20.0f, IsOnFire()); //left foot
	EntityToWorldSpace(Vector(0, -4, 3), &vecOut);
	CreateRagGib(szHGibNames[3], vecOut, GetAbsAngles(), vecDamageDir, 20.0f, IsOnFire()); //right foot
	EntityToWorldSpace(Vector(0, 4, 10), &vecOut);
	CreateRagGib(szHGibNames[4], vecOut, GetAbsAngles(), vecDamageDir, 20.0f, IsOnFire()); //left lower leg
	EntityToWorldSpace(Vector(0, -4, 10), &vecOut);
	CreateRagGib(szHGibNames[5], vecOut, GetAbsAngles(), vecDamageDir, 20.0f, IsOnFire()); //right lower leg
	EntityToWorldSpace(Vector(0, 4, 20), &vecOut);
	CreateRagGib(szHGibNames[6], vecOut, GetAbsAngles(), vecDamageDir, 20.0f, IsOnFire()); //left upper leg
	EntityToWorldSpace(Vector(0, -4, 20), &vecOut);
	CreateRagGib(szHGibNames[7], vecOut, GetAbsAngles(), vecDamageDir, 20.0f, IsOnFire()); //right upper leg*/
	for (int i = 0; i < 14; i++)
	{
		int iBone = LookupBone(szHGibBones[i]);
		GetBonePosition(iBone, vecOrigin, vecAngle);
		if ((vecOrigin == vec3_origin) || (vecOrigin == GetAbsOrigin()))
		{
			Warning("WARNING: Gibbed %s is missing bone %s for creating gib!\n", GetClassname(), szHGibBones[i]);
			continue;
		}
		//CBaseEntity *pGib = CreateRagGib(szHGibNames[i], vecOrigin, vecAngle, vecDamageDir, 15.0f, IsOnFire());
		CGiblet* pGib = static_cast<CGiblet*>(CBaseEntity::CreateNoSpawn("giblet", vecOrigin, vecAngle));
		Assert(pGib);
		pGib->SetModel(szHGibNames[i]);
		pGib->SetLocalAngles(GetLocalAngles() + QAngle(-180, 180, 180));
		//pGib->SetAbsVelocity(vecDamageDir);
		//pGib->SetAbsAngularVelocity(QAngle(random->RandomFloat(-60, 60), random->RandomFloat(-60, 60), random->RandomFloat(-60, 60)));
		pGib->SetBleedTimes(random->RandomInt(5, 15));
		pGib->SetBloodColor(BloodColor());
		DispatchSpawn(pGib);
		pGib->SetNextThink(gpGlobals->curtime + random->RandomFloat(0.0f, 0.5f));
		...
		if (pPhys)
		{
			pPhys->ApplyForceCenter(info.GetDamageForce() * 0.50f);
			//pPhys->SetMass(pPhys->GetMass() * 10);
			AngularImpulse angImp;
			angImp = RandomAngularImpulse(-1024, 1024);
			pPhys->SetVelocity(NULL, &angImp);
			if (bDissolve)
			{
				float flDrag = 0.2;
				pPhys->EnableGravity(false);
				pPhys->SetDragCoefficient(&flDrag, NULL);
			}
		}
	}
